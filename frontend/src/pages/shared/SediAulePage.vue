  ...
  <div class="page-sedi">
    <div class="page-header d-flex flex-wrap gap-3 align-items-center mb-4">
      ...
      <div class="ms-auto d-flex gap-2 align-items-center">
        <input v-model="dataConsulta" type="date" class="form-control form-control-sm"
          style="width:auto" :min="oggiISO" @change="caricaDisponibilita" />
    ...
    <div v-else>
      <div v-for="sede in sedi" :key="sede.id" class="mb-4">
        <div class="sede-header d-flex align-items-center gap-2 mb-3">
          <svg class="icon icon-primary"><use :href="sprites + '#it-map-marker'"></use></svg>
          <h4 class="mb-0 fw-bold">{{ sede.nome }}</h4>
          <small class="text-muted ms-1">{{ sede.citta }}</small>
          <span class="badge bg-primary-subtle text-primary ms-1">{{ auleDiSede(sede.id).length }} aule</span>
        </div>
            ...
            <div class="card border-0 shadow-sm h-100 aula-card">
              <div class="card-header bg-white border-bottom-0 pb-0 d-flex justify-content-between align-items-start">
                ...
                <span class="badge rounded-pill"
                  :class="aulaLibera(aula.id) ? 'bg-success' : 'bg-danger'">
                  {{ aulaLibera(aula.id) ? 'Libera' : 'Occupata' }}
                </span>
              </div>
              ...
              <div class="card-body pt-2">
                ...
                <div class="mb-3">
                  <div class="d-flex justify-content-between mb-1">
                    <small class="text-muted">Occupazione {{ dataConsulta }}</small>
                ...
                <div class="slot-grid mb-3">
                  <div v-for="h in oreGiornata" :key="h"
                    class="slot" :class="slotOccupato(aula.id, h) ? 'slot--occupato' : 'slot--libero'"
              ...
              <div class="card-footer bg-white border-0 pt-0">
                <RouterLink v-if="aulaLibera(aula.id)"
                  ...
                  Prenota per {{ dataConsulta }}
                </RouterLink>
                <span v-else class="d-block text-center text-danger small py-1">
                  Aula occupata in questa data
                </span>
              </div>
            </div>
          </div>
...
// ── Estrae gli slot confermati per un'aula nella data selezionata ─────────────
// La risposta backend è: [{ id, aula_id, corso_id, stato, slots: [{data, ora_inizio, ora_fine}] }]

const eventiPerAula = computed(() => {
  // map: aula_id → array di { prenId, slotIdx, corsoId, oraInizio, oraFine, oraH }
  const map = {}
...
function prenotazioniAula(aulaId) {
  return eventiPerAula.value[aulaId] || []
}
...
function aulaLibera(aulaId) {
  return (eventiPerAula.value[aulaId] || []).length === 0
}
...
.sede-header { padding-bottom: .5rem; border-bottom: 2px solid #e8eaf0; }
.aula-card { border-radius: 12px; }
.slot-grid { display: flex; gap: 3px; flex-wrap: wrap; }
.slot { width: 28px; height: 28px; border-radius: 4px; display: flex; align-items: center; justify-content: center; cursor: default; }
.slot-label { font-size: .6rem; font-weight: 600; }
.slot--libero   { background: #d4edda; color: #155724; }
.slot--occupato { background: #f8d7da; color: #721c24; }
.prenotazione-chip {
  display: flex; align-items: center; background: #f0f4ff;
  border-left: 3px solid #0066cc; border-radius: 4px;
  padding: 3px 8px; font-size: .78rem; margin-bottom: 4px; overflow: hidden;
}
</style>