            ...
            <div class="row g-3 align-items-stretch">
              ...
              <div class="col-md-5">
                <div class="conflitto-pill pill-a h-100">
                  <div class="small fw-semibold text-muted mb-2">Prenotazione A — #{{ c.prenotazione_id_1 }}</div>
                  <template v-for="s in [infoSlot(c, 1)]">
                    <template v-if="s">
                      <div class="fw-bold">{{ formatData(s.data) }}</div>
                      <div class="text-nowrap">{{ s.ora_inizio }} – {{ s.ora_fine }}</div>
                      <div class="mt-1 small">
                        <strong>{{ nomeAulaFn(prenById(c.prenotazione_id_1)?.aula_id) }}</strong>
                        <span class="text-muted ms-1">{{ sedeDiAulaFn(prenById(c.prenotazione_id_1)?.aula_id) }}</span>
                      </div>
                      <div class="small text-muted mt-1">
                        Corso {{ prenById(c.prenotazione_id_1)?.corso_id }}
                        <span v-if="prenById(c.prenotazione_id_1)?.tipo === 'massiva'" class="badge bg-info ms-1">Ricorrente</span>
                      </div>
                    </template>
              ...
              <div class="col-md-5">
                <div class="conflitto-pill pill-b h-100">
                  <div class="small fw-semibold text-muted mb-2">Prenotazione B — #{{ c.prenotazione_id_2 }}</div>
                  <template v-for="s in [infoSlot(c, 2)]">
                    <template v-if="s">
                      <div class="fw-bold">{{ formatData(s.data) }}</div>
                      <div class="text-nowrap">{{ s.ora_inizio }} – {{ s.ora_fine }}</div>
                      <div class="mt-1 small">
                        <strong>{{ nomeAulaFn(prenById(c.prenotazione_id_2)?.aula_id) }}</strong>
                        <span class="text-muted ms-1">{{ sedeDiAulaFn(prenById(c.prenotazione_id_2)?.aula_id) }}</span>
                      </div>
                      <div class="small text-muted mt-1">
                        Corso {{ prenById(c.prenotazione_id_2)?.corso_id }}
                        <span v-if="prenById(c.prenotazione_id_2)?.tipo === 'massiva'" class="badge bg-info ms-1">Ricorrente</span>
                      </div>
                    </template>
                    <div v-else class="text-muted small">Caricamento...</div>
                  </template>
...
.conflitto-pill { padding: 12px 14px; border-radius: 8px; font-size: .82rem; line-height: 1.5; }
